...
(function(){
  ...
  var loadmeta = function(loadinto,v){
    ...
    */
    ...
    $('#adv_get_articles').off('click').on('click',function(){
      var t = $(this);
      var add_select = function(d){
        var toappend='';
        var select=$('<select />',{}).change(function(){
          var t=$(this);
          $('#metafield_value').val(t.val());
        }).html('<option value="">Add Article handle as value</option>');

        for (var i = 0, len = d.articles.length; i < len; i++) {
          toappend+='<option value="'+d.articles[i].handle+'">'+d.articles[i].title+'</option>';
        }
      ...
      if(!_data('pages')){
        $.ajax({
          ...
          error: function(){
            notice('Failed to load articles',true);
          }
        });
      }else{
        var d = _data('articles');
        add_select(d);
      }
      return false;
